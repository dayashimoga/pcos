...
void main() {
  ...
  group('FileBloc', () {
    ...
    blocTest<FileBloc, FileState>(
      ...
      build: () {
        when(() => mockRepo.listRoot()).thenAnswer((_) async => {
          'entries': [
            {'id': '1', 'name': 'Documents', 'entry_type': 'folder'},
            {'id': '2', 'name': 'photo.jpg', 'entry_type': 'file', 'size_bytes': 1024},
          ],
          'path': [],
          'total': 2,
        });
    ...
    blocTest<FileBloc, FileState>(
      ...
      build: () {
        when(() => mockRepo.listFolder('folder-1')).thenAnswer((_) async => {
          'entries': [],
          'path': [{'id': 'folder-1', 'name': 'Documents'}],
          'total': 0,
        });
        return FileBloc(fileRepository: mockRepo);
      },
    ...
    blocTest<FileBloc, FileState>(
      ...
      build: () {
        when(() => mockRepo.createFolder('New Folder', null)).thenAnswer((_) async => {'id': '3', 'name': 'New Folder'});
        when(() => mockRepo.listRoot()).thenAnswer((_) async => {'entries': [], 'path': [], 'total': 0});
        return FileBloc(fileRepository: mockRepo);
      },
      act: (bloc) => bloc.add(const FolderCreateRequested(name: 'New Folder')),
      expect: () => [isA<FileActionSuccess>(), isA<FileLoading>(), isA<FileLoaded>()],
    );
