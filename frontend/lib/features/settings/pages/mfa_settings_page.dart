...
class _MfaSettingsPageState extends State<MfaSettingsPage> {
  ...
  Future<void> _checkStatus() async {
    try {
      ...
      setState(() {
        _mfaEnabled = resp.data['mfa_enabled'] == true;
        _loading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Future<void> _verifyCode() async {
    ...
    try {
      final api = getIt<ApiClient>();
      await api.dio.post('/api/v1/auth/mfa/verify',
    ...
    final code = await showDialog<String>(
        ...
        builder: (ctx) {
          ...
          return AlertDialog(
            ...
            content: TextField(
                controller: ctrl,
                decoration: const InputDecoration(labelText: 'Enter TOTP code'),
    ...
    return SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Protect your account with TOTP-based MFA',
            ...
            Container(
              ...
              child: Row(children: [
                ...
                if (!_mfaEnabled && _secret == null)
                  ElevatedButton(
                      onPressed: _setupMfa, child: const Text('Enable MFA')),
              ]),
            ),
            ...
            if (_secret != null) ...[
              ...
              Container(
                ...
                child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Step 1: Add to your authenticator app',
                          style: TextStyle(
                              fontSize: 14,
                      ...
                      const Text('Step 2: Enter the 6-digit code',
                          style: TextStyle(
                              fontSize: 14,
                              fontWeight: FontWeight.w600,
                              color: AppTheme.textPrimary)),
