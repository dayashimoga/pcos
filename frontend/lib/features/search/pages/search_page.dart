...
class _SearchPageState extends State<SearchPage> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Search failed: $e'), backgroundColor: AppTheme.error),
        );
      }
    }
  }
  ...
  List<Map<String, dynamic>> get _filteredResults {
    ...
    if (_sortBy == 'name') {
      ...
      list.sort((a, b) => ((b['size_bytes'] ?? 0) as int).compareTo((a['size_bytes'] ?? 0) as int));
    }
    return list;
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: 8, runSpacing: 8, children: [
          _FilterChip(label: 'All', selected: _typeFilter == 'all', onTap: () { setState(() => _typeFilter = 'all'); if (_searched) _search(); }),
          _FilterChip(label: 'Files', icon: Icons.insert_drive_file_rounded, selected: _typeFilter == 'file', onTap: () { setState(() => _typeFilter = 'file'); if (_searched) _search(); }),
          _FilterChip(label: 'Folders', icon: Icons.folder_rounded, selected: _typeFilter == 'folder', onTap: () { setState(() => _typeFilter = 'folder'); if (_searched) _search(); }),
          const SizedBox(width: 8),
          _FilterChip(label: 'By Name', icon: Icons.sort_by_alpha_rounded, selected: _sortBy == 'name', onTap: () => setState(() => _sortBy = _sortBy == 'name' ? 'relevance' : 'name')),
          _FilterChip(label: 'By Size', icon: Icons.storage_rounded, selected: _sortBy == 'size', onTap: () => setState(() => _sortBy = _sortBy == 'size' ? 'relevance' : 'size')),
        ]),
        ...
        if (!_loading && _searched && results.isEmpty)
          Container(
            ...
            child: Column(children: [
              ...
              const SizedBox(height: 8),
              Text('Try different keywords or filters', style: TextStyle(fontSize: 13, color: AppTheme.textMuted.withOpacity(0.7))),
            ]),
          ),
            ...
            child: Column(children: [
              Padding(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  ...
                  Text(
                    _sortBy == 'relevance' ? 'By relevance' : _sortBy == 'name' ? 'A → Z' : 'Largest first',
              ...
              ...results.map((e) => ListTile(
                leading: Icon(
                  e['entry_type'] == 'folder' ? Icons.folder_rounded : Icons.insert_drive_file_rounded,
                ...
                subtitle: Text(
                  e['entry_type'] == 'file' ? formatFileSize(e['size_bytes'] ?? 0) : 'Folder',
                  style: const TextStyle(fontSize: 12, color: AppTheme.textMuted),
                ),
                trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                  if (e['entry_type'] == 'file')
                    IconButton(
                      icon: const Icon(Icons.link_rounded, size: 18, color: AppTheme.textMuted),
                      tooltip: 'Copy link',
                      onPressed: () {
                        final api = getIt<ApiClient>();
    ...
    child: InkWell(
      ...
      child: AnimatedContainer(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(label, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: selected ? AppTheme.primary : AppTheme.textMuted)),
        ]),
      ),
    ),
  );
