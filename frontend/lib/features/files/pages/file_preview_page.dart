...
class FilePreviewPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(_name, style: const TextStyle(fontSize: 15, color: Colors.white, fontWeight: FontWeight.w500)),
        actions: [
          IconButton(
  ...
  Widget _buildPreview(BuildContext context) {
    if (_mime.startsWith('image/')) return _ImagePreview(url: _downloadUrl, name: _name);
    if (_mime == 'application/pdf') return _PdfPreview(url: _downloadUrl);
    if (_mime.startsWith('video/')) return _VideoPreview(url: _downloadUrl, mime: _mime);
    if (_mime.startsWith('audio/')) return _AudioPreview(url: _downloadUrl, name: _name);
    if (_mime.startsWith('text/') || _mime.contains('json') || _mime.contains('xml') || _mime.contains('yaml')) {
  ...
  Widget build(BuildContext context) => InteractiveViewer(
    ...
    child: Center(
      child: Image.network(url,
        ...
        errorBuilder: (_, __, ___) => Column(mainAxisSize: MainAxisSize.min, children: [
  ...
  Widget build(BuildContext context) => Column(
    children: [
      Container(
        ...
        child: Row(children: [
          const Icon(Icons.picture_as_pdf_rounded, size: 20, color: AppTheme.error),
          const SizedBox(width: 10),
          const Expanded(child: Text('PDF Preview', style: TextStyle(fontSize: 14, color: AppTheme.textPrimary, fontWeight: FontWeight.w500))),
          TextButton.icon(
            onPressed: () {},
        ...
        child: Container(
          ...
          child: Center(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.picture_as_pdf_rounded, size: 80, color: Colors.white24),
              const SizedBox(height: 16),
              const Text('PDF Viewer', style: TextStyle(fontSize: 18, color: Colors.white70, fontWeight: FontWeight.w600)),
              const SizedBox(height: 8),
              Text(url, style: const TextStyle(fontSize: 11, color: Colors.white30), overflow: TextOverflow.ellipsis),
              const SizedBox(height: 20),
              const Text('For web: Use HtmlElementView to embed PDF.\nFor mobile: Use a PDF rendering package.',
...
class _VideoPreviewState extends State<_VideoPreview> {
  ...
  Widget build(BuildContext context) => Column(children: [
    ...
    Container(
      ...
      color: AppTheme.surface,
      child: Row(children: [
        const Icon(Icons.videocam_rounded, size: 20, color: AppTheme.accent),
        const SizedBox(width: 10),
        Expanded(child: Text('Video Player (${widget.mime})', style: const TextStyle(fontSize: 14, color: AppTheme.textPrimary, fontWeight: FontWeight.w500))),
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
          decoration: BoxDecoration(color: AppTheme.accent.withOpacity(0.15), borderRadius: BorderRadius.circular(6)),
          child: const Text('HLS Ready', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: AppTheme.accent)),
        ),
      ]),
    ),
    // Video area
              ...
              GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  child: Icon(
                    ...
                    size: _playing ? 28 : 36, color: Colors.white,
                  ),
                ),
              ),
              // Status text
    ...
    Container(
      ...
      child: Row(children: [
        IconButton(
          icon: Icon(_playing ? Icons.pause_rounded : Icons.play_arrow_rounded, color: Colors.white, size: 24),
          onPressed: () => setState(() => _playing = !_playing),
        ),
        Expanded(child: SliderTheme(
          data: SliderThemeData(thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 6), trackHeight: 3, activeTrackColor: AppTheme.primary, inactiveTrackColor: Colors.white12, thumbColor: AppTheme.primary),
          child: Slider(value: 0, onChanged: (_) {}),
        )),
        const Text('0:00 / --:--', style: TextStyle(fontSize: 12, color: Colors.white54)),
        const SizedBox(width: 8),
        IconButton(icon: const Icon(Icons.fullscreen_rounded, color: Colors.white54, size: 22), onPressed: () {}),
        IconButton(icon: const Icon(Icons.volume_up_rounded, color: Colors.white54, size: 22), onPressed: () {}),
      ]),
    ),
  ]);
}
    ...
    child: Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(
          ...
          child: const Icon(Icons.audiotrack_rounded, size: 48, color: AppTheme.primary),
        ),
        const SizedBox(height: 20),
        Text(name, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppTheme.textPrimary), textAlign: TextAlign.center),
        const SizedBox(height: 20),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          IconButton(icon: const Icon(Icons.skip_previous_rounded, color: Colors.white54), onPressed: () {}),
          Container(
          ...
          IconButton(icon: const Icon(Icons.skip_next_rounded, color: Colors.white54), onPressed: () {}),
        ]),
      ]),
    ),
  );
...
class _UnsupportedPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      const Icon(Icons.insert_drive_file_rounded, size: 64, color: Colors.white24),
      const SizedBox(height: 16),
      Text(name, style: const TextStyle(fontSize: 16, color: Colors.white70, fontWeight: FontWeight.w500)),
      const SizedBox(height: 8),
      Text('Preview not available for $mime', style: const TextStyle(fontSize: 13, color: Colors.white38)),
...
class _TextPreviewState extends State<_TextPreview> {
  ...
  Future<void> _load() async {
    try {
      ...
      setState(() { _content = resp.data.toString(); _loading = false; });
    } catch (e) {
      setState(() { _content = 'Error: $e'; _loading = false; });
    }
  }
