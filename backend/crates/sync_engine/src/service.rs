...
pub async fn get_changes(pool: &PgPool, user_id: Uuid, since: Option<chrono::DateTime<chrono::Utc>>) -> AppResult<Vec<ChangeEvent>> {
    ...
    Ok(rows.into_iter().map(|(id, fid, name, _, ts)| ChangeEvent {
        id, file_entry_id: fid, change_type: "modified".to_string(), name, version: 1, timestamp: ts,
    }).collect())
}

pub async fn sync_status(pool: &PgPool, user_id: Uuid, device_id: Uuid) -> AppResult<SyncStatusResponse> {
    ...
    let (conflicts,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM sync_states WHERE user_id = $1 AND device_id = $2 AND status = 'conflict'")
        .bind(user_id).bind(device_id).fetch_one(pool).await.unwrap_or((0,));
    let last_sync: Option<(chrono::DateTime<chrono::Utc>,)> = sqlx::query_as("SELECT MAX(last_synced_at) FROM sync_states WHERE user_id = $1 AND device_id = $2")
        .bind(user_id).bind(device_id).fetch_optional(pool).await.ok().flatten();

    Ok(SyncStatusResponse { device_id, total_synced: synced, pending, conflicts, last_sync: last_sync.map(|l| l.0) })
}

pub async fn resolve_conflict(pool: &PgPool, user_id: Uuid, req: ResolveConflictRequest) -> AppResult<()> {
...
pub async fn add_sync_folder(pool: &PgPool, user_id: Uuid, req: AddSyncFolderRequest) -> AppResult<SyncFolder> {
...
pub async fn list_sync_folders(pool: &PgPool, user_id: Uuid) -> AppResult<Vec<SyncFolder>> {
    let folders = sqlx::query_as::<_, SyncFolder>("SELECT * FROM sync_folders WHERE user_id = $1 ORDER BY created_at DESC")
        .bind(user_id).fetch_all(pool).await?;
    Ok(folders)
}

pub async fn remove_sync_folder(pool: &PgPool, user_id: Uuid, folder_id: Uuid) -> AppResult<()> {
    let r = sqlx::query("DELETE FROM sync_folders WHERE id = $1 AND user_id = $2").bind(folder_id).bind(user_id).execute(pool).await?;
    if r.rows_affected() == 0 { return Err(AppError::NotFound("Sync folder not found".to_string())); }
    Ok(())
}
