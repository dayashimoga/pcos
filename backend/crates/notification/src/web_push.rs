...
pub async fn subscribe(pool: &PgPool, user_id: Uuid, req: SubscribeRequest, user_agent: Option<String>) -> AppResult<PushSubscription> {
...
pub async fn send_push(pool: &PgPool, user_id: Uuid, payload: &PushPayload) -> AppResult<u32> {
    ...
    let mut sent = 0u32;
    for sub in &subs {
        match client.post(&sub.endpoint)
