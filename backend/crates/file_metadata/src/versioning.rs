...
pub async fn download_version(
    ...
    // Verify ownership
    let file: (String, Option<String>) =
        sqlx::query_as("SELECT name, mime_type FROM file_entries WHERE id = $1 AND user_id = $2")
            .bind(file_id)
            .bind(auth.claims.sub)
            .fetch_optional(pool)
            .await?
    ...
    let data = storage
        .read_file(&version.storage_path)
        .await
