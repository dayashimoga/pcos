...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/api/v1/folders",
            post(handlers::create_folder).get(handlers::list_root),
        )
        .route(
            "/api/v1/folders/:id",
            get(handlers::list_folder)
                .put(handlers::rename_item)
        ...
        .route(
            ...
            post(versioning::restore_version),
        )
        .route(
            "/api/v1/files/:file_id/versions/:version_id/download",
            get(versioning::download_version),
        )
