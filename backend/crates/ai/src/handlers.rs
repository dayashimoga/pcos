...
pub async fn auto_tag(
    State(state): State<AppState>, auth: AuthUser, Json(req): Json<TagRequest>,
) -> Result<impl IntoResponse, AppError> {
    let provider = AiProvider::from_env().ok_or_else(|| AppError::Internal("AI not configured".to_string()))?;
...
pub async fn find_duplicates(
    ...
    Ok(Json(serde_json::json!({ "duplicates": groups, "total_groups": groups.len() })))
}
...
pub async fn smart_search(
    State(state): State<AppState>, auth: AuthUser, Query(params): Query<SmartSearchQuery>,
) -> Result<impl IntoResponse, AppError> {
...
pub async fn ai_status(
    State(_state): State<AppState>, auth: AuthUser,
) -> Result<impl IntoResponse, AppError> {
