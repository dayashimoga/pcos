...
pub async fn list_jobs(
    ...
    let jobs: Vec<serde_json::Value> = rows
        .into_iter()
        .map(|(id, jtype, status, details, ts)| {
...
pub async fn job_stats(
    ...
    let (total,): (i64,) = sqlx::query_as("SELECT COUNT(*) FROM jobs WHERE user_id = $1")
        ...
        .unwrap_or((0,));
    let (running,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM jobs WHERE user_id = $1 AND status = 'running'")
            .bind(auth.claims.sub)
            .fetch_one(s.db.pool())
            .await
            .unwrap_or((0,));
    let (completed,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM jobs WHERE user_id = $1 AND status = 'completed'")
            .bind(auth.claims.sub)
            .fetch_one(s.db.pool())
            .await
            .unwrap_or((0,));
    let (failed,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM jobs WHERE user_id = $1 AND status = 'failed'")
            .bind(auth.claims.sub)
            .fetch_one(s.db.pool())
            .await
            .unwrap_or((0,));
    Ok(Json(
        serde_json::json!({ "total": total, "running": running, "completed": completed, "failed": failed }),
    ))
}
