...
pub async fn create_share(pool: &PgPool, user_id: Uuid, req: CreateShareRequest) -> AppResult<ShareLink> {
    // Verify file ownership
    let _file = sqlx::query_as::<_, (Uuid,)>("SELECT id FROM file_entries WHERE id = $1 AND user_id = $2")
        .bind(req.file_entry_id).bind(user_id)
    ...
    let expires_at = req.expires_in_hours.map(|h| chrono::Utc::now() + chrono::Duration::hours(h));
...
pub async fn list_shares(pool: &PgPool, user_id: Uuid) -> AppResult<Vec<ShareLink>> {
    let shares = sqlx::query_as::<_, ShareLink>("SELECT * FROM share_links WHERE user_id = $1 ORDER BY created_at DESC")
...
pub async fn get_share(pool: &PgPool, user_id: Uuid, share_id: Uuid) -> AppResult<ShareLink> {
    sqlx::query_as::<_, ShareLink>("SELECT * FROM share_links WHERE id = $1 AND user_id = $2")
        .bind(share_id).bind(user_id)
        .fetch_optional(pool).await?
        .ok_or_else(|| AppError::NotFound("Share not found".to_string()))
}

pub async fn update_share(pool: &PgPool, user_id: Uuid, share_id: Uuid, req: UpdateShareRequest) -> AppResult<ShareLink> {
    let mut share = get_share(pool, user_id, share_id).await?;

    if let Some(perm) = &req.permission { share.permission = perm.clone(); }
    if let Some(active) = req.is_active { share.is_active = active; }
    if let Some(max) = req.max_downloads { share.max_downloads = Some(max); }

    let pwd_hash = match &req.password {
        Some(p) => Some(hash_password(p).map_err(|e| AppError::Internal(e.to_string()))?),
        None => share.password_hash.clone(),
    };
    let expires = req.expires_in_hours.map(|h| chrono::Utc::now() + chrono::Duration::hours(h)).or(share.expires_at);
...
pub async fn delete_share(pool: &PgPool, user_id: Uuid, share_id: Uuid) -> AppResult<()> {
    let result = sqlx::query("DELETE FROM share_links WHERE id = $1 AND user_id = $2")
        .bind(share_id).bind(user_id).execute(pool).await?;
    if result.rows_affected() == 0 { return Err(AppError::NotFound("Share not found".to_string())); }
    Ok(())
}

pub async fn access_shared(pool: &PgPool, token: &str, password: Option<&str>) -> AppResult<SharedFileInfo> {
    let share = sqlx::query_as::<_, ShareLink>("SELECT * FROM share_links WHERE token = $1 AND is_active = true")
        .bind(token).fetch_optional(pool).await?
        .ok_or_else(|| AppError::NotFound("Share link not found or expired".to_string()))?;

    // Check expiration
    if let Some(exp) = share.expires_at {
        if chrono::Utc::now() > exp { return Err(AppError::Forbidden("Share link has expired".to_string())); }
    }
    // Check max downloads
    if let Some(max) = share.max_downloads {
        if share.download_count >= max { return Err(AppError::Forbidden("Download limit reached".to_string())); }
    }
    // Check password
    ...
    ).bind(share.file_entry_id).fetch_one(pool).await?;

    Ok(SharedFileInfo { name: file.0, entry_type: file.1, mime_type: file.2, size_bytes: file.3, permission: share.permission })
}
...
pub async fn download_shared(pool: &PgPool, token: &str) -> AppResult<(String, Vec<u8>)> {
    ...
    if let Some(exp) = share.expires_at {
        if chrono::Utc::now() > exp { return Err(AppError::Forbidden("Expired".to_string())); }
    }
    if let Some(max) = share.max_downloads {
        if share.download_count >= max { return Err(AppError::Forbidden("Limit reached".to_string())); }
    }

    // Increment download count
    sqlx::query("UPDATE share_links SET download_count = download_count + 1 WHERE id = $1")
        .bind(share.id).execute(pool).await?;

    let (name, storage_path): (String, Option<String>) = sqlx::query_as(
        "SELECT name, storage_path FROM file_entries WHERE id = $1"
    ).bind(share.file_entry_id).fetch_one(pool).await?;
