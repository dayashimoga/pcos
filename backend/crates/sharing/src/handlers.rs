...
pub async fn create_share(State(state): State<AppState>, auth: AuthUser, Json(req): Json<CreateShareRequest>) -> Result<impl IntoResponse, AppError> {
...
pub async fn list_shares(State(state): State<AppState>, auth: AuthUser) -> Result<impl IntoResponse, AppError> {
    ...
    Ok(Json(serde_json::json!({ "shares": responses, "total": responses.len() })))
}

pub async fn get_share(State(state): State<AppState>, auth: AuthUser, Path(id): Path<Uuid>) -> Result<impl IntoResponse, AppError> {
    let share = service::get_share(state.db.pool(), auth.claims.sub, id).await?;
    let base_url = std::env::var("PCOS_PUBLIC_URL").unwrap_or_default();
    Ok(Json(share.to_response(&base_url)))
}

pub async fn update_share(State(state): State<AppState>, auth: AuthUser, Path(id): Path<Uuid>, Json(req): Json<UpdateShareRequest>) -> Result<impl IntoResponse, AppError> {
    let share = service::update_share(state.db.pool(), auth.claims.sub, id, req).await?;
    let base_url = std::env::var("PCOS_PUBLIC_URL").unwrap_or_default();
    Ok(Json(share.to_response(&base_url)))
}

pub async fn delete_share(State(state): State<AppState>, auth: AuthUser, Path(id): Path<Uuid>) -> Result<impl IntoResponse, AppError> {
...
pub async fn access_shared(
    State(state): State<AppState>, Path(token): Path<String>, Query(q): Query<SharedAccessQuery>,
) -> Result<impl IntoResponse, AppError> {
    let info = service::access_shared(state.db.pool(), &token, q.password.as_deref()).await?;
    Ok(Json(info))
}

pub async fn download_shared(State(state): State<AppState>, Path(token): Path<String>) -> Result<impl IntoResponse, AppError> {
    ...
    if let Some(exp) = share.expires_at {
        if chrono::Utc::now() > exp { return Err(AppError::Forbidden("Share link expired".to_string())); }
    }
    ...
    // Get file info
    let (name, storage_path, mime_type): (String, Option<String>, Option<String>) = sqlx::query_as(
        "SELECT name, storage_path, mime_type FROM file_entries WHERE id = $1"
    ).bind(share.file_entry_id).fetch_one(pool).await?;

    let path = storage_path.ok_or_else(|| AppError::Internal("File has no storage path".to_string()))?;
