...
pub async fn create_share(
    State(state): State<AppState>, auth: AuthUser, Json(req): Json<CreateShareRequest>,
...
pub async fn update_share(
    State(state): State<AppState>, auth: AuthUser, Path(id): Path<Uuid>, Json(req): Json<UpdateShareRequest>,
) -> Result<impl IntoResponse, AppError> {
...
pub async fn download_shared(
    State(state): State<AppState>, Path(token): Path<String>,
) -> Result<impl IntoResponse, AppError> {
    ...
    #[derive(sqlx::FromRow)]
    struct FileInfo { name: String, storage_path: Option<String>, mime_type: Option<String> }
    ...
    let path = file.storage_path.ok_or_else(|| AppError::Internal("File has no storage path".to_string()))?;
